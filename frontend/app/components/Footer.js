...
export default function Footer() {
  const footerLinks = {
    ...
    businessLinks: [
      { href: "/business-signup", label: "Sign Up" },
      { href: "/business-faq", label: "Business FAQ" },
      { href: "/success-stories", label: "Success Stories" },
      { href: "/business-resources", label: "Resources" },
    ]
  };
  ...
  const SocialIcon = ({ name, icon }) => (
    <a href="#" className="bg-gray-800 text-gray-400 p-2 rounded border border-gray-700 hover:text-blue-400 transition-colors">
    ...
    <footer className="bg-gray-900 text-white py-12 border-t border-gray-700">
      <div className="max-w-7xl mx-auto px-8">
        <div className="grid md:grid-cols-4 gap-8">
          {/* Column 1 - About */}
          <div>
            <FooterHeading title="NowOrNever" />
            <p className="text-gray-400">
              Reducing food waste and fighting hunger in Chicago, one meal at a time.
            </p>
          </div>
          ...
          <div>
            <FooterHeading title="For Businesses" />
            <ul className="space-y-2">
              {footerLinks.businessLinks.map((link) => (
                <FooterLink key={link.href} href={link.href} label={link.label} />
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <form className="flex">
              <input 
                ...
                className="px-4 py-2 rounded-l w-full bg-gray-800 border border-gray-700 text-gray-300 focus:outline-none focus:border-blue-500" 
              />
              <button 
                type="submit" 
                className="bg-blue-600 text-white px-4 py-2 rounded-r border border-blue-500 hover:bg-blue-700 transition-colors"
              >
                Subscribe
              </button>
            </form>
        ...
        <div className="mt-8 pt-8 border-t border-gray-700 text-center text-gray-400">
          <p>&copy; {new Date().getFullYear()} NowOrNever. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
